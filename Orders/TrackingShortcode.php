...
class TrackingShortcode
{
    ...
    {
        $content = '<h2 class="moova-tracking-form-title">' . __('Numero de seguimiento', 'wc-moova') . '</h2>
        ...
        if ($tracking_statuses === false) {
            $content .= '<h3  class="moova-tracking-error">' . __('Hubo un error, por favor intente nuevamente', 'wc-moova') . '</h3>';
        } else {
            if (!empty($tracking_statuses)) {
                $content .= '<h3>' . __('Numero de seguimiento', 'wc-moova') . ': ' . $moova_id . '</h3>';
                $content .= '<table>';
                $content .= '<tr>';
                $content .= '<th width="30%">' . __('Fecha', 'wc-moova') . '</th>';
                $content .= '<th width="70%">' . __('Estado', 'wc-moova') . '</th>';
                $content .= '</tr>';
